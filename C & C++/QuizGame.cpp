...
// 4. Using a constant for easy maintenance
const int MAX_RANGE = 1000;

bool isPrime(int num) {
  // 2. Making the function mathematically safe for all inputs
  if (num <= 1) {
    return false;
  }
...
int main() {
  ...
  std::cout << "*** Finding prime numbers within given range ***\n";
  std::cout << "Enter a number to define range of numbers(max: " <<
MAX_RANGE <<
"): ";
  ...
  if (num > MAX_RANGE) {
    std::cout << "Invalid Input! Input must not be greater than " <<
MAX_RANGE
<< ".\n"; return 1;
  }

  if (num <= 1) {
    std::cout << "Invalid Input! Input must be greater than 1.\n";
    return 1;
  }

  // 1. Changed to <= to include the user's upper limit
  for (int i = 2; i <= num; i++) {
    if (isPrime(i)) {
      std::cout << i << " ";
    }
    // 3. Removed the useless else { continue; } block
  }
